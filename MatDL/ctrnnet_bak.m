...
layers_size=[10 50];
TS=0.5;
[model, opt] = init_two_ctrnn(4, 2, layers_size, opt);
...
x = X(1:size(X,1)/2, :, :);
y = Y(1:size(Y,1)/2, :, :);
maxRelError = gradcheck(@two_ctrnn, x, model, y, opt, 2); %last argument should be less or euqa to the number of classes
