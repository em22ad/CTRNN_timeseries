...
QUANT_LVLS=3;
%Choose dimensions in the dims=[Dim1 Dim2 Dim3]; or dims=[Dim1 Dim2] format
dims=[1 2]; 
...
Xin1=ACC1(1:20598,dims); % EXP 1 USR 1
Xin2=ACC2(1:19286,dims); % EXP 2 USR 1
Xin4=ACC4(1:16565,dims); % EXP 4 USR 2
Xin5=ACC5(1:20994,dims); % EXP 5 USR 3
Xin7=ACC7(1:17668,dims); % EXP 7 USR 4
Xin9=ACC9(1:16864,dims); % EXP 9 USR 5
...
if (QUANT_LVLS==2)
	for di=1:size(dims,2)
		T(di,:)=return_pattern(Xin(di,:));
    end
	if (QUANT_LVLS==3)
		for di=1:size(dims,2)
			T(di,:)=return_pattern_3(Xin(di,:));
		end
end
%%

Tp1=zeros(size(dims,2),size(Xin1,1));
Tp2=zeros(size(dims,2),size(Xin2,1));
Tp4=zeros(size(dims,2),size(Xin4,1));
Tp5=zeros(size(dims,2),size(Xin5,1));
Tp7=zeros(size(dims,2),size(Xin7,1));
Tp9=zeros(size(dims,2),size(Xin9,1));

%%7 STAND_TO_SIT 
%EXP 1 USR 1 ACT 7
Tp1(:,1233:round((1233+1392)/1))=1;
%%EXP 2 USR 1 ACT 7
Tp2(:,1227:round((1227+1432)/1))=1;
%%EXP 4 USR 2 ACT 7
Tp4(:,1352:round((1352+1511)/1))=1;
%%EXP 5 USR 3 ACT 7
Tp5(:,1365:round((1365+1506)/1))=1;
%%EXP 7 USR 4 ACT 7
Tp7(:,1292:round((1292+1527)/1))=1;
%%EXP 9 USR 5 ACT 7
Tp9(:,1222:round((1222+1386)/1))=1;
...
for i=1:WINDOW_SZ:size(Xin,2)-WINDOW_SZ
    if mod(counter,3) ~= 0
        ...
        if (sum(Tp(1,i:i+WINDOW_SZ)) >= WINDOW_SZ/3.0)
            if (USE_QUANT == 0)
                X_f(ctr_a,:,:)=Xin(:,i:i+WINDOW_SZ-1)';
            else
                X_f(ctr_a,:,:)=T(:,i:i+WINDOW_SZ-1)';
            end
            Y_f=[Y_f;[1 0]];
            ctr_a=ctr_a+1;
        ...
        %if (sum(Tp(1,i:i+WINDOW_SZ)) >= WINDOW_SZ/3.0)
            if (USE_QUANT == 0)
                XVal_f(ctr_b,:,:)=Xin(:,i:i+WINDOW_SZ-1)';
            else
                XVal_f(ctr_b,:,:)=T(:,i:i+WINDOW_SZ-1)';
            end
            YVal_f=[YVal_f;[1 0]];        
            ctr_b=ctr_b+1;
...
%% Initialize model
%X = permute(X, dims);
%XVal=permute(XVal, dims);
opt = struct;
%layers_size=[5 5];
layers_size=[size(dims,2)];
TS=0.01;
